...
let
  ...
  ofaceSet = toNftSet (map (x: ''"${x}"'') cfg.externalInterfaces);
  oifExpr = optionalString (cfg.externalInterfaces != [ ]) ''oifname { ${ofaceSet} }'';
  iofaceSet = lists.intersectLists cfg.internalInterfaces cfg.externalInterfaces;
  ...
  mkTable =
    ...
    ''
      ...
      chain post {
        ...
        ${
          concatMapStrings (face: ''
            iifname ${face} oifname ${face} accept;
          '') iofaceSet
        }
        ${
