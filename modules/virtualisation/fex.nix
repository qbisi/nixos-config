...
let
      ...
      config = {
        wrappedThunk = wrapThunk {
          ...
          hostRpath =
            ...
            in
            lib.concatStringsSep ":" (packagePaths ++ config.extraHostPaths);
        ...
        overlayPaths = lib.map (x: "${x.path}/${x.name}") (
          lib.cartesianProduct {
            ...
            path = lib.map mkLibPath' (
              (lib.concatMap (set: config.packages set ++ config.guestPackages set) cfg.guestPackageSets)
              ++ config.extraGuestPaths
            );
          }
        );
      };
  ...
  convertConfig = lib.mapAttrs (
    _: value:
    if builtins.isBool value then
      (if value then "1" else "0")
    else if builtins.isList value then
      builtins.map convertConfig value
    else if builtins.isAttrs value then
      builtins.mapAttrs (_: convertConfig) value
    else
      toString value
  );
in

{
  ...
  config = lib.mkIf cfg.enable {
    ...
    virtualisation.fex.forwardedLibraries = {
      libGL.names = enumSuffixes "libGL.so" [
        ".1"
        ".1.2.0"
        ".1.7.0"
      ];
      ...
      libdrm.names = enumSuffixes "libdrm.so" [
        ".2"
        ".2.4.0"
        ".2.124.0"
      ];
